...
#     idx = np.argmin(dist)
#     tmp[i, 12] = vicon_data[i, 9]
#     tmp[i, 13] = vicon_data[i, 10]
#     tmp[i, 14] = vicon_data[i, 11]
# ctrl_data = tmp
...
plt.plot(ctrl_ts[:-1], (ctrl_data[1:,6]-ctrl_data[:-1,6])/0.033, label='finite_diff_v')
...
plt.figure(1)
# plt.plot(ctrl_ts, ctrl_data[:,0], label='prediction_on')
plt.plot(ctrl_ts, -ctrl_data[:,2], label='current_y')
plt.plot(ctrl_ts, -ctrl_data[:,5], label='target_y')
# plt.plot(ctrl_ts, ctrl_data[:,6]-ctrl_data[:,3], label='leader_y')
plt.plot(ctrl_ts, ctrl_data[:,9], label='vicon_y')
plt.plot(vicon_ts, vicon_data[:,3], label='vicon_chaser')
plt.plot(vicon_ts, vicon_data[:,9], label='vicon_leader')
# plt.plot(ctrl_ts, ctrl_data[:,9])
# plt.plot(ctrl_ts, np.abs(ctrl_data[:,3]-ctrl_data[:,9]), label='diff')
# plt.plot(ctrl_ts, ctrl_data[:,13]*30)
plt.plot(pf_ts, pf_data[:,4], label='pf_v')
plt.plot(pf_ts, pf_data[:,6], label='mode')
# plt.plot(pf_ts, pf_data[:,7], label='prediciton_count')
...
plt.plot(vicon_ts, vicon_data[:,11], label='vicon_leader')
