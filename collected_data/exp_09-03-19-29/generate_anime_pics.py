...
def plot_dashed_segments(fig: pv.Plotter, start, end, color='black', line_width=2):
    dashed_segments = create_dashed_line(start, end)
    for seg_start, seg_end in dashed_segments:
        line = pv.Line(seg_start, seg_end) 
        fig.add_mesh(line, color = color, line_width = line_width)
    return fig 
...
pf = pd.read_csv(fn2)
fn3 = os.path.join(script_dir, 'exp_09-03-19-29.csv')
...
fn7 = os.path.join(script_dir, 'exp_09-03-19-29.xcp')
xcp = open(fn7, 'r')
xcpstring = xcp.read()
...
for i in range(1,len(vicon_ts_segment)-1):
    # if np.isnan(dist_diff[i]):
    #     dist_diff[i] = (dist_diff[i-1]+dist_diff[i+1])/2
    print(np.linalg.norm(vicon_data_segment[i,(3,4,5,9,10,11)]-vicon_data_segment[i-1,(3,4,5,9,10,11)]))
    if (not (-3.3<vicon_data_segment[i,4]<-2.7)) and (-3.3<vicon_data_segment[i-1,4]<-2.7):
        jump_start = i 
        jump_end = i+1
        # in_range = True 
    elif not (-3.3<vicon_data_segment[i,4]<-2.7):
        jump_end = jump_end+1 
    
    if (not (-3.3<vicon_data_segment[i,4]<-2.7)) and (-3.3<vicon_data_segment[i+1,4]<-2.7):
        val_start = vicon_data_segment[jump_start-1,:]
        val_end = vicon_data_segment[jump_end,:]
        vals = np.linspace(val_start, val_end, jump_end-jump_start+2)
        vicon_data_segment[jump_start:jump_end,:] = vals[1:-1,:]
    # if in_range and np.linalg.norm(vicon_data_segment[i,(3,4,5,9,10,11)]-vicon_data_segment[i+1,(3,4,5,9,10,11)])>1.0:
    #     val_start = vicon_data_segment[jump_start-1,:]
    #     val_end = vicon_data_segment[i+1,:]
    #     vals = np.linspace(val_start, val_end, i+1-jump_start+2)
    #     vicon_data_segment[jump_start:i+1,:] = vals[1:-1,:]
    #     in_range = False
...
plotter.add_mesh(line_leader, color = 'blue', point_size=6)
plotter.add_mesh(line_chaser, color = 'blue', point_size=6)

# line_ellip = pv.PolyData(ellip)
for i in range(80, len(vicon_ts_segment), 200):
    print(i)
    ...
    if i==0:
        ...
        color = 'orange'
    plotter.add_mesh(leader_mesh, color=color)

    # plotter = plot_dashed_segments(plotter, vicon_data_segment[i, 9:12], vicon_data_segment[i, 3:6])
    # line = pv.Line(vicon_data_segment[i, 9:12], vicon_data_segment[i, 3:6]) 
