...
data_fns = get_files_with_extension(data_dir, 'bag')
all_output_dir = os.path.join(script_dir, 'data_processed')
...
for data_fn in data_fns:
    ...
    #     os.mkdir(img_dir)
    dt = data_fn.split('.')[0]
    output_dir = os.path.join(all_output_dir, dt)    
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    rosbag_to_csv(fn, output_dir)
