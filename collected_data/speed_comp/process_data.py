...
def find_tightest_separator(x, y):
    ...
    c[1] = 1  # Minimize -c for tightest fit
    # c[0] = x[-1]
    # Constraints
    A = np.zeros((n, 2))
    A[:, 0] = -x  # For lambda
...
for sub_folder in sub_folders:
    ...
    if len(column_headings)<10:
        ...
        idx_start, idx_end = find_index_range(vicon_ts, [start_t, end_t])
        idx_start += 5
        ...
        plt.figure(1)
        plt.plot(vicon_ts[idx_start:idx_end], array, label=f"{sub_folder[10:]}_orig")
        ...
        # plt.plot(ctrl_ts, ctrl_data[:,8], label=f'{sub_folder[10:]}_vicon_x')
        ...
        # plt.figure(1)
        # # plt.plot(ctrl_ts, ctrl_data[:,0], label='prediction_on')
        # # plt.plot(ctrl_ts, ctrl_data[:,3], label=f'{sub_folder[10:]}_current_y')
        # # plt.plot(ctrl_ts, ctrl_data[:,6], label=f'{sub_folder[10:]}_target_y')
        # # plt.plot(ctrl_ts, ctrl_data[:,6]-ctrl_data[:,3], label='leader_y')
        # # plt.plot(ctrl_ts, ctrl_data[:,9], label=f'{sub_folder[10:]}_vicon_y')
        # plt.plot(vicon_ts, vicon_data[:,9], label=f'{sub_folder[10:]}_vicon_chaser')

        # plt.legend()

        # plt.figure(2)
        # # plt.plot(ctrl_ts, ctrl_data[:,0], label='prediction_on')
        # plt.plot(ctrl_ts, ctrl_data[:,4], label=f'{sub_folder[10:]}_current_z')
        # plt.plot(ctrl_ts, ctrl_data[:,7], label=f'{sub_folder[10:]}_target_z')
        # # plt.plot(ctrl_ts, ctrl_data[:,7]-ctrl_data[:,4], label='leader_z')
        # plt.plot(ctrl_ts, ctrl_data[:,10], label=f'{sub_folder[10:]}_vicon_z')
        # plt.legend()
