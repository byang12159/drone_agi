...
pf = pd.read_csv(fn2)
fn3 = os.path.join(script_dir, 'exp_08-20-14-07.csv')
...
fn7 = os.path.join(script_dir, 'exp_08-20-14-07.xcp')
xcp = open(fn7, 'r')
xcpstring = xcp.read()
...
for i in range(vicon_ts.shape[0]):
    ...
    if chaser_y>0.45 and chaser_y<0.55 and chaser_z>0.95 and chaser_z<1.05:
        ts = vicon_ts[i]
        break
start_ts = ts-3.75
end_ts = ts+45
...
print(f'ASDT: {(45-total_time)/9}')
print(f'max_dist: {(dist_diff-1).max()}')
