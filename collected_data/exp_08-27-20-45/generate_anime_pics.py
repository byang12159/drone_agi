...
pf = pd.read_csv(fn2)
fn3 = os.path.join(script_dir, 'exp_08-27-20-45.csv')
...
fn7 = os.path.join(script_dir, 'exp_08-27-20-45.xcp')
xcp = open(fn7, 'r')
xcpstring = xcp.read()
...
# Compute offset between vio pose and vicon pose
pos_diff = np.zeros((100,3))
for i in range(100):
    idx = np.random.choice(np.arange(len(vicon_ts_segment)))
    ts = vicon_ts_segment[idx]
    vicon_pos = vicon_data_segment[idx, 9:]
    idx_ctrl = (np.abs(ctrl_ts-ts)).argmin()
    current_pos = ctrl_data[idx_ctrl,2:5]
    current_pos = current_pos[[1,0,2]]
    current_pos[1] = -current_pos[1]
    pos_diff[i,:] = current_pos-vicon_pos
...
for i in range(vicon_ts_segment.shape[0]):
    ...
    idx = (np.abs(ctrl_ts-ts)).argmin()
    tmp = ctrl_data[idx,(3,2,4)]
    tmp[1] = -tmp[1]
    target_list.append(tmp-pos_diff)
...
vicon_data_segment[:, 9] += 0.3
vicon_data_segment[:,10] -=0.5
for i in range(1440, 1790, 20):
    print(i)
    ...
    # target_pos = pv.PolyData(target_list[i,:])
    # plotter.add_mesh(target_pos, color = 'orange', point_size=10)
    # tmp = 0 if i-100<0 else i-100
    # line_target = pv.PolyData(target_list[tmp:i,:])
    # line_target.lines = np.hstack(([target_list[tmp:i,:].shape[0]], np.arange(target_list[tmp:i,:].shape[0])))
    # plotter.add_mesh(line_target, color = 'orange', point_size=2

    line_leader  = pv.PolyData(vicon_data_segment[1430:i,3:6])
    line_leader.lines = np.hstack(([vicon_data_segment[1430:i,:].shape[0]], np.arange(vicon_data_segment[1430:i,:].shape[0])))
    line_chaser  = pv.PolyData(vicon_data_segment[1430:i,9:12])
    line_chaser.lines = np.hstack(([vicon_data_segment[1430:i,:].shape[0]], np.arange(vicon_data_segment[1430:i,:].shape[0])))
    ...
    # # plt.imshow(cv_img)
    # # plt.show()
    cv2.imwrite(f"frames_img/frame_{i:05d}.png",picam_img)
    # del line_ellip
    # del line_leader
    # del line_chaser
    # del leader_mesh
