...
pf = pd.read_csv(fn2)
fn3 = os.path.join(script_dir, 'exp_08-25-16-42.csv')
...
fn7 = os.path.join(script_dir, 'exp_08-25-16-42.xcp')
xcp = open(fn7, 'r')
xcpstring = xcp.read()
...
for i in range(1, len(dist_diff)):
    ...
    if dist_diff[i]>displacement+0.5 and i+1>=len(dist_diff):
        ts_start = tmp_ts[sees-1]
        ts_end = tmp_ts[seee-1]
        total_time = total_time + (ts_end-ts_start)
    elif dist_diff[i]>displacement+0.5 and not dist_diff[i+1]>displacement+0.5:
        ts_start = tmp_ts[sees-1]
        ts_end = tmp_ts[seee]
        total_time = total_time + (ts_end-ts_start)
print(f"frac chaser away: {total_time/45}")

tmp = copy.deepcopy(dist_diff[:4854])
