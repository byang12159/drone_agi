...
pf = pd.read_csv(fn2)
fn3 = os.path.join(script_dir, 'exp_09-17-21-57.csv')
...
fn7 = os.path.join(script_dir, 'exp_09-17-21-57.xcp')
xcp = open(fn7, 'r')
xcpstring = xcp.read()
...
for i in range(len(tmp_ts)):
    if tmp[i,7] >1 and i == 0:
        sees = i+1
        seee = i+1 
    elif tmp[i,7] > 1 and tmp[i-1,7]<=1:
        sees = i 
        seee = i+1
    elif tmp[i,7] > 1:
        seee += 1
    if tmp[i,7] > 1 and i+1 >=len(tmp_ts):
