...
class PID_Controller:
    ...
    def callback_detection_bool(self, data):
        global prediction_on,prediction_count, data_storage,vicon_pose, target_pose,new_message_received,mode_backoff, current_pose
        if data.data:
            prediction_on = False
            prediction_count = 0   
            ...
            if prediction_count >= 60 and mode_backoff==False:
                # First time
                # target_vicon_point = np.array([-1.6,0.06,1.3])
                displacement = np.array([-2.4, -2.5, 0])
    ...
    #         if la.norm(current_pose -active_target_pose) < 0.3:
    #             break
    ...
    # def move_pos(self):
    # def move_pos(self):
    #     global vicon_pose, current_pose
    #     end_deploy = False
        
    #     force_hover_msg = Empty()
    #     self.pub_hover.publish(force_hover_msg)
    #     self.rate.sleep()

    #     while not end_deploy:
    #         print("IN MOVE POS")
    #         target_vicon_point = np.array([-1.6,0.06,1.3])
    #         displacement = target_vicon_point-vicon_pose
    ...
    #         self.pub_pose.publish(target)

    #         print("move pos publishing: ",target)
    #         self.rate.sleep() 
