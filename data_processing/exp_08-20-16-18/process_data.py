...
pf = pd.read_csv(fn2)
fn3 = os.path.join(script_dir, 'exp_08-20-16-18.csv')
...
fn7 = os.path.join(script_dir, 'exp_08-20-16-18.xcp')
xcp = open(fn7, 'r')
xcpstring = xcp.read()
...
plt.plot(pf_ts, pf_data[:,2], label='kf_pos')
plt.plot(pf_ts, pf_data[:,3], label='kf_v')
plt.plot(pf_ts, pf_data[:,6], label='mode')
plt.plot(vicon_ts, vicon_data[:,5], label='vicon_leaderpos')
plt.plot(pf_ts, pf_data[:,15], label='pf_pos')
plt.plot(pf_ts, pf_data[:,16], label='pf_v')
# plt.plot(pf_ts, pf_data[:,7], label='prediciton_count')
...
ax.plot(vicon_data[:,3], vicon_data[:,4], vicon_data[:,5], label='drone_lead')
ax.plot(vicon_data[:,9], vicon_data[:,10], vicon_data[:,11], label='drone_chase')
ax.set_xlabel('x')
ax.set_xlabel('y')
ax.set_xlabel('z')
ax.legend()
